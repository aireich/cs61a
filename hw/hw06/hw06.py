...
def is_bst(t):
    ...
    def helper(t, result):
        if len(t.branches) > 2:
            ...
            if t.label >= bst_min(t) and t.label < bst_max(t):
                for b in t.branches:
                    result = helper(b, result) and result
...
def store_digits(n):
    ...
    def first_digit(n):
        return int (n // pow(10, int (math.log10(n))))
    
    def remains(n):
        return int (n % pow(10, int (math.log10(n))))
    
    link = Link(first_digit(n))
    n = remains(n)
    ...
    def helper(n, link):
        if n // 10 == 0:
            ...
            new_link = Link(first_digit(n))
            link.rest = new_link
            return helper(remains(n), new_link)
...
def path_yielder(t, value):
    ...
    if t.label == value:
        yield [value]
    for b in t.branches:
        for ele in path_yielder(b, value):
            yield [t.label] + ele
