...
from sre_parse import expand_template  # Python 2 compatibility
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        "*** YOUR CODE HERE ***"
        curent_frame = self
        while curent_frame is not None:
            if symbol in curent_frame.bindings.keys():
                return curent_frame.bindings[symbol]
            else:
                curent_frame = curent_frame.parent
        # END PROBLEM 2
    ...
    def make_child_frame(self, formals, vals):
        ...
        current_name = formals
        current_val = vals
        while current_name is not nil and current_val is not nil:
            new_frame.define(current_name.first, current_val.first)
            current_name = current_name.rest
            current_val = current_val.rest
        if current_name is not nil or current_val is not nil:
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            python_args = []
            current_arg = args
            while current_arg is not nil:
                python_args.append(current_arg.first)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        #       Pair('x', nil)), 
        #     Pair(
        #         Pair('+', Pair('x', Pair(2, nil))), nil))
        name = target.first
        formals = target.rest
        validate_formals(formals) 
        body = expressions.rest
        proced = LambdaProcedure(formals, body,env)
        env.define(name, proced)
        return name
        # END PROBLEM 9
    else:
