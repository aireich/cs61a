    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.can_contain(self):
                ...
                self.contain_ant(place.ant) #cannot switch sequence
                place.ant = self #muts be kept
            else:
    ...
    def nearest_bee(self, beehive):
        ...
        while current_place is not None:
            if current_place is beehive:
                return None
            if place_count < self.min_range:
                current_place = current_place.entrance
                place_count += 1
            elif place_count > self.max_range:
                return None
            elif len(current_place.bees) == 0:
                current_place = current_place.entrance
                place_count += 1
            else:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor > amount:
            self.place.bees = change_bees(amount)
        else:
            self.place.bees = change_bees(amount + self.damage)
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        super().__init__(armor)
        self.impostor = QueenAnt.queen_count > 0
            ...
            while current_place is not None:
                if current_place.ant is not None:
                    ...
                    if isinstance(ant , ContainerAnt) \
                        and ant.contained_ant is not None \
                        and ant.contained_ant not in self.buffed:
                        ant.contained_ant.damage *= 2
                        self.buffed.append(ant.contained_ant)
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if target and target not in ScaryThrower.scared:
            target.statuses.append(2)
            apply_status(make_scare, target, 2)
            ScaryThrower.scared.append(target)
        # END Problem EC
...
class LaserAnt(ThrowerAnt):
        ...
        while current_place is not beehive:
            if current_place.ant is not None:
                ...
                if ant is not self:
                    result[ant] = distance
                if isinstance(ant, ContainerAnt):
                    result[ant.contained_ant] = distance
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        result = self.damage - distance * 0.2 - 0.05 * self.insects_shot
        if result > 0:
            return result
        else:
            return 0
